...
local VERIFY_CHECKS = {
    ...
    },
    {
        ...
        steps = "On a character with no snapshot yet, open the Enhance tab and read the line at the top. Then do the same on a character with no crafting professions at all - a fresh alt is ideal.",
        expect = "It names the professions you actually have: 'Open Leatherworking or Blacksmithing once'. On the gatherer it says none of your professions makes anything that goes on gear, rather than sending you to open something.",
        broke = "The names come from GetNumSkillLines/GetSkillLineInfo, which returns NOTHING when the skill headers are collapsed - that quirk is why the manual profession overrides in Settings exist at all. The code treats an empty read as 'I could not see' and falls back to the generic sentence, so the failure to watch for is the generic sentence appearing on a character who plainly has Enchanting. If that happens, collapse/expand your skill headers and see if it changes: that tells you whether Ascension has the same quirk, and the Settings override is the workaround either way.",
    },
    {
        id = "enhancesnapshot", since = "0.181.0a",
        gate = "tools/enhancesnapshot.js",
        title = "A profession window read once is still known after you close it",
        steps = "Open Enchanting (or any crafting profession). CLOSE it. Walk somewhere else, then open the Enhance tab. Then run /valuate enhancecheck.",
        expect = "The tab lists enhancements with a line saying where they came from and how long ago - 'Read from Enchanting (34), 2 minutes ago'. enhancecheck reports the same book under 'Remembered from earlier'.",
        broke = "This is the half of the feature that was missing until v0.181.0a: GetNumCrafts and GetNumTradeSkills only answer while their window is OPEN, so the tab used to work in exactly one order and look empty in every other. The book is now read on TRADE_SKILL_SHOW / CRAFT_SHOW and written to saved variables. Two ways it can still fail on this client, and they look different: nothing is remembered at all (the events do not fire, or the list is empty for longer than one tick - enhancecheck will say 'Nothing remembered yet'), or it is remembered but empty (the apis answer with zero rows even while open, which enhancecheck shows as '0 open right now' with the window up). The second is the interesting one.",
    },
    {
        id = "enhancescroll", since = "0.178.0a",
        gate = "tools/enhancepanel.js",
        title = "The Enhance tab scrolls to its last row",
        steps = "Open the Enhance tab and scroll to the bottom. Check the seventeenth slot is fully reachable. Then drag the window taller and scroll again.",
        expect = "Every row can be reached, and the scroll bar disappears entirely if the whole list already fits.",
        broke = "The scroll range is content height minus frame height, and the window animates to its tab height AFTER the refresh that computed it - so it used to be set to whatever fitted the PREVIOUS tab, leaving the last row or two unreachable. It is recomputed on OnSizeChanged now. Only the client has real font metrics: the harness models wrapping with a rough character count, so a row that wraps to three lines in game and two in the model is exactly the case no gate can see.",
    },
    {
