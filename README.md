...
> **This is a fork.** Branch `claude-fork`, currently **v0.194.0a**, substantially diverged
...
/valuate why <item>       explain this item: best-in-slot, roll decision, arrow, junk
...
Developed without the game running. **68 subsystems execute real Lua** headlessly against a
...
in 122 specific ways and requires the matching gate to fail for each. A `SURVIVED` line means an
...
exactly 100 (against which "gap ÷ best" and "gap ÷ 100" are the same arithmetic, so the
percentage was untested). It edits real files, restores them byte-for-byte, and is deliberately
**not** part of `gates.js`.
...
68 of them **execute real Lua** under fengari against a mocked WoW API (listed under *Status*
...
in-game. It holds **41 checks** — it was short once and has grown with the addon, so it now
