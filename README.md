...
> **This is a fork.** Branch `claude-fork`, currently **v0.98.0a**, substantially diverged
...
Developed without the game running. **33 subsystems execute real Lua** headlessly against a
...
in 29 specific ways and requires the matching gate to fail for each. A `SURVIVED` line means an
...
33 of them **execute real Lua** under fengari against a mocked WoW API (listed under *Status*
...
in-game. It holds **32 checks** — it was short once and has grown with the addon, so it now
