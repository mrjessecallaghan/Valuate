...
> **This is a fork.** Branch `claude-fork`, currently **v0.53.1a**, substantially diverged
...
Developed without the game running. 20 subsystems are executed headlessly against a mocked
...
comparison text, the upgrade-arrow driver, the stat-share ranking, the Settings keybind capture, tab switching, the confirm dialog, the minimap button, the stat search, the character-sheet score, the icon picker, surplus-gear marking, and the auto-roll decision.
...
20 of them **execute real Lua** under fengari against a mocked WoW API (listed under *Status*
